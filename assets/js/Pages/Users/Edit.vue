<template>
	<div>
		...
		<div class="flex justify-start mb-8 max-w-3xl">
			...
			<img v-if="user.photo_path" class="block ml-4 w-8 h-8 rounded-full" :src="`/uploads/${user.photo_path}`" />
		</div>
		<div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
			<form @submit.prevent="update">
				...
				<div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
					<button v-if="!user.deleted_at && (!user.owner && user.id !== $page.props.auth.user.id)" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete User</button>
